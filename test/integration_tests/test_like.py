...
class LikeTest(unittest.TestCase):
    ...
    def test_like_fails_on_non_string_col(self):
        ...
        select_query = """SELECT * FROM MemeImages JOIN LATERAL OCRExtractor(data) AS X(label, x, y) WHERE label LIKE "[A-Za-z]*CANT";"""
        with self.assertRaises(Exception):
            batch_response = execute_query_fetch_all(select_query)
